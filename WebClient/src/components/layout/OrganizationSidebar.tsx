...
import {LayoutGrid,Settings,ClipboardList ,Users,Building ,Bell} from "lucide-react";
...
const ManagementSidebar = () => {
    const menuItems = [
        ...
        { href: `/organization/notifications`, label: "Notifications", icon: <Bell className="group-hover:text-white size-5" /> },
          { href: `/organization/settings`, label: "Settings", icon: <Settings className="group-hover:text-white size-5" /> },
    ];
        ...
        <div className="relative flex h-screen">
            ...
            <aside className={`fixed z-20 top-0 left-0 h-full bg-gray-900 text-white w-64 p-6 transform transition-transform duration-300 ease-in-out ${navOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
                <nav className="flex flex-col justify-between h-full">
                    <div>
                        <h2 className="text-xl font-bold mb-8">organization name</h2>
                    ...
                    <button
                        ...
                        aria-label="Logout"
                    >
                        Your dashboard &nbsp; <i className="fa-solid fa-right-from-bracket"></i>
                    </button>
                    </Link>
                </nav>
            </aside>
