...
const OrganizationForm = () => {
    ...
    const verifyOrganization = useCallback(async (orgId: string) => {
        try {
            ...
            if (res.data.departments?.length > 0) {
                ...
                toast.error("No departments found for this organization.");
                return false;
            }
        } catch (err) {
            console.error(err);
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        setDisabled(true);
        try {
            const response = await axios.post("/api/departments/join", {
                    ...
                    {step === 1 && (
                        <>
                            ...
                            <button
                                ...
                                onClick={goNext}
                                disabled={disabled}
                                className={`w-full text-white font-semibold py-2 rounded-lg transition ${disabled
